                    ...
                    for(auto &I : BB) {
                        ...
                        if(auto *SI = dyn_cast<StoreInst>(&I)) {
                            if(auto *BI = dyn_cast<BitCastInst>(SI->getPointerOperand())) {
                                if(auto *GI = dyn_cast<GetElementPtrInst>(BI->getOperand(0))) {
                                    if(GI->getPointerOperand()->getName().startswith("__live_out")) {
...
X("pasha-count-ins", "Count instances of different instruciton types");
