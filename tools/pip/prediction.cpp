...
struct PredictionBuffer {
  ...
  PredictionBuffer(size_t historySize, PathID target)
    : buffer{new IDCount[historySize + 1]},
      start{0},
      end{0},
      count{0},
      target{target},
      capacity{historySize + 1}
      { }
  ...
  std::tuple<std::string, PathID, bool> takeSample() {
    ...
    while (historyIndex < 2*(capacity - 1)) {
      ...
      if (historyIndex == 2*(capacity - 1)) {
        auto &next = remaining ? idCount : buffer[(bufferIndex+1) % capacity];
        follower   = next.first;
      }
    ...
    return std::make_tuple(std::string{reinterpret_cast<char*>(history),
...
void
printHistory(const std::string &history) {
...
main(int argc, char **argv) {
  ...
  std::unordered_map<std::string, std::vector<std::pair<PathID,uint64_t>>> samples;
  ...
  while(trackedCount < THRESHOLD && (std::cin >> idString >> count)) {
    ...
    while (buffer.hasSample()) {
      ...
      auto found = std::find_if(followers.begin(), followers.end(),
  ...
  for (auto &historyFollowers : samples) {
    ...
    auto maxPath = std::max_element(followers.begin(), followers.end(),
      [] (std::pair<PathID,uint64_t> &path1, std::pair<PathID,uint64_t> &path2) {
#ifdef BENCH_GCC
    ...
    uint64_t total = std::accumulate(followers.begin(), followers.end(), 0,
    ...
    printHistory(std::string{reinterpret_cast<char*>(&predicted), 2*sizeof(uint64_t)});
    printf("\n");
  }
