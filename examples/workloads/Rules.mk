...
.setup.success: 
	@rm -rf $(FUNCTION)
	@mkdir $(FUNCTION)
	llvm-link $(BC) $(BITCODE_REPO)/$(LLVM_VERSION)/lib/m.bc -o $(FUNCTION)/$(NAME).bc
	opt -O2 -disable-loop-vectorization -disable-slp-vectorization  $(FUNCTION)/$(NAME).bc -o $(FUNCTION)/$(NAME).bc
	touch .setup.success
	@echo SETUP
...
epp-run: epp-inst .epp-run.success
.epp-run.success:
	cd $(FUNCTION) && \
